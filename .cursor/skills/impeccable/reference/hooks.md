...
2. Invoke the admin script and pass the user's output through verbatim:
   ...
   ```bash
   .cursor/skills/impeccable/scripts/impeccable hooks <action> [args...]
   ```
...
```bash
.cursor/skills/impeccable/scripts/impeccable hooks ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example self-served exception, with the evidence named:

```bash
.cursor/skills/impeccable/scripts/impeccable hooks ignore-value bounce-easing bounce-ball --shared --reason "Agent: literal ball-bounce animation, bounce easing is the subject"
```

Example whole-rule font exception:

```bash
.cursor/skills/impeccable/scripts/impeccable hooks ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```
...
```bash
.cursor/skills/impeccable/scripts/impeccable hooks ignore-file "src/legacy/Card.tsx"
```
